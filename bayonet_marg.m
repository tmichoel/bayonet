...
% at x=xbay(j), we know solution
% xb = xhat(idx);
% uh = uhat(idx);
%[xb,uh] =  bayonet(A(:,idx),w(idx)-xbay(j)*C(idx,j),mu,tau,xbay(idx));
[xb,uh] =  bayonet_mean(C(idx,idx),w(idx)-xhat(j)*C(idx,j),mu,tau,xhat(idx));
%logZx(numx+1) = bayonet_norm(C(idx,idx),w(idx)-xbay(j)*C(idx,j),mu,tau,xb,uh);
...
%    logZx(k) = bayonet_norm(C(idx,idx),w(idx)-x(k)*C(idx,j),mu,tau,xb,uh);
    logZx(k) = bayonet_norm(y-x(k)*A(:,j),A(:,idx),lambda,mu,tau,xb,uh);
end

% go down from x=xbay(j)
xb = xhat(idx);
for k=numx:-1:1
    [xb,uh] =  bayonet_mean(C(idx,idx),w(idx)-x(k)*C(idx,j),mu,tau,xb);
%    logZx(k) = bayonet_norm(C(idx,idx),w(idx)-x(k)*C(idx,j),mu,tau,xb,uh);
